...
// 用法：
//   node 工作文件/校验脚本/counts.mjs          默认：重生成 CLAUDE.md「计数区」（两个注释标记之间）
//   node 工作文件/校验脚本/counts.mjs --check  核对：计数区与磁盘不符则 exit 1
...
const 根 = (p) => 取条目(p)
const 镜像基 = join('工作文件', '发布', 'github备份')
const 内视 = !existsSync(join(ROOT, 镜像基))          // 在镜像仓库内运行：镜像行即本仓库自身，不单列
const 镜像 = (p) => 取条目(join(镜像基, p))
...
const 规则表 = [
  ...
  ['理论存档', () => 根(join('工作文件', '理论存档')),
    (e) => {
      const md = 文件数(e, '.md')
      const 其他 = e.filter(([n, d]) => !d && !n.endsWith('.md')).length
      return [`${md} 篇${其他 ? ` ${md + 其他} 文件` : ''}`, '工作文件/理论存档/*（md 计篇，docx 等计其他）']
    }],
  ['理论备份', () => 根(join('工作文件', '理论备份')),
    (e) => [`${文件数(e, '.md')} 文件`, '工作文件/理论备份/*.md']],
  ['工作日志', () => 根(join('工作文件', '工作日志')),
    (e) => [`${e.filter(([, d]) => !d).length} 件`, '工作文件/工作日志/* 全部文件']],
  ['历史史论正文', () => 根(join('工作文件', '历史史论')),
    (e) => {
      const 正文 = 正文篇(e)
      const 总史论 = 正文.filter(([n]) => n.includes('总史论')).length
      return [`${正文.length} 篇（总史论 ${总史论}＋分论 ${正文.length - 总史论}）`, '工作文件/历史史论/*.md 减配套CLAUDE.md']
    }],
  ['计划与方案', () => 根(join('工作文件', '计划与方案')),
    (e) => {
      const 顶层 = e.filter(([, d]) => !d).length
      const 研究 = 根(join('工作文件', '计划与方案', 'UBIApp经济影响研究')).filter(([, d]) => !d).length
      return [`${顶层} 文件＋UBIApp研究 ${研究} 件`, '工作文件/计划与方案 顶层全部文件＋UBIApp经济影响研究/*']
    }],
  ['校验脚本', () => 根(join('工作文件', '校验脚本')),
    (e) => [`${e.filter(([, d]) => !d).length} 件`, '工作文件/校验脚本/* 全部文件（含本脚本）']],
  ...(内视 ? [] : [
    ['镜像·社会主义论', () => 镜像('社会主义论'),
      (e) => {
        const n = 文件数(e, '.md')
    ...
    ['镜像·理论存档', () => 镜像(join('工作文件', '理论存档')),
      (e) => {
        const md = 文件数(e, '.md')
        const 其他 = e.filter(([n, d]) => !d && !n.endsWith('.md')).length
        return [`${md} 篇${其他 ? ` ${md + 其他} 文件` : ''}`, '同理论存档规则']
      }],
    ['镜像·理论备份', () => 镜像(join('工作文件', '理论备份')),
      (e) => [`${文件数(e, '.md')} 文件`, '同理论备份规则']],
    ['镜像·历史史论', () => 镜像(join('工作文件', '历史史论')),
      (e) => {
        const 正文 = 正文篇(e)
        const 总史论 = 正文.filter(([n]) => n.includes('总史论')).length
        return [`${正文.length} 篇（总史论 ${总史论}＋分论 ${正文.length - 总史论}）`, '同历史史论规则']
      }],
    ['镜像·计划与方案', () => 镜像(join('工作文件', '计划与方案')),
      (e) => {
        const 顶层 = e.filter(([, d]) => !d).length
        const 研究 = 镜像(join('工作文件', '计划与方案', 'UBIApp经济影响研究')).filter(([, d]) => !d).length
        return [`${顶层} 文件＋UBIApp研究 ${研究} 件`, '同计划与方案规则']
      }],
    ['镜像·工作日志', () => 镜像(join('工作文件', '工作日志')),
...
export function 核对计数区() {
  ...
  if (开头 === -1 && 结尾 === -1)
    return { 过: false, 详情: 'CLAUDE.md 无计数区标记（跑 node 工作文件/校验脚本/counts.mjs 生成）' }
  if (开头 === -1 || 结尾 === -1)
